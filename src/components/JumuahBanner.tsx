...
export const JumuahBanner: React.FC<JumuahBannerProps> = ({
  ...
  return (
    <div className="w-full max-w-full overflow-hidden bg-gradient-to-r from-amber-950 via-slate-950 to-orange-950 border-b border-amber-500/30 text-amber-100 px-3 sm:px-4 py-2 sm:py-2.5 shadow-md">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-2.5 text-xs w-full">
        {/* Left message */}
        <div className="flex items-center gap-2.5 text-center md:text-left min-w-0">
          ...
          <div className="min-w-0">
            <p className="font-black text-white flex flex-wrap items-center gap-1.5 justify-center md:justify-start">
              <span>Règle Spéciale Vendredi &bull; Prière du Jumu&apos;ah</span>
              <span className="px-1.5 py-0.2 rounded bg-amber-500/20 text-amber-300 text-[10px] uppercase font-bold border border-amber-500/30 whitespace-nowrap">
                11h00 ➔ 15h00
              </span>
            </p>
            <p className="text-[11px] text-amber-200/80 line-clamp-2 sm:line-clamp-none">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 shrink-0">
          ...
          >
            <span>{simulatedFridayPause ? "🧪 Pause Active" : "🧪 Test Vendredi"}</span>
          </button>

          {onOpenScheduleOrder && (
            <button
              onClick={onOpenScheduleOrder}
              className="px-3 py-1 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 text-slate-950 font-black text-[11px] hover:opacity-90 transition cursor-pointer shadow-sm whitespace-nowrap"
            >
